...
public class TagStructure {
    private static final Comparator<TagController> CONTROLLER_COMPARATOR = Comparator.comparingInt
            (TagController::getPriority);
    ...
    public TagStructure(TagImpl tag, Multimap<Player, RenderedTagLine> visible) {
    ...
    public Stream<Map.Entry<Player, Collection<StructureTransaction>>> addTagController(TagController controller,
        ...
        for (TagController.TagLine line : controller.getFor(tag.getTarget())) {
            added.put(lineIndex);
    ...
    public Stream<Map.Entry<Player, Collection<StructureTransaction>>> removeTagController(TagController controller, Stream<Player> players) {
        int index = -1;
        for (int i = 0; i < lines.size(); i++)
