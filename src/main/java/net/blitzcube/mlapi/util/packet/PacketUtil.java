...
public class PacketUtil {
    ...
    public static void updateWatcher(WrappedDataWatcher metadata, Map<Integer, Object> pendingChanges) {
        for (Map.Entry<Integer, Object> e : pendingChanges.entrySet()) {
            if (e.getValue().getClass() == org.bukkit.inventory.ItemStack.class) {
                ...
                try {
                    metadata.setObject(e.getKey(), WrappedDataWatcher.Registry.getItemStackSerializer(false),
            ...
            metadata.setObject(e.getKey(), WrappedDataWatcher.Registry.get(e.getValue().getClass()), e.getValue());
        }
    ...
    public static void sendFriendly(PacketContainer packet, Player[] forWho) {
        for (Player p : forWho) {
            ...
            try {
                ...
                plugin.getLogger().warning("Failed to send packet of type " + packet.getType().name() + " to player "
                        + p.getName());
                e.printStackTrace();
            }
        }
    ...
    private static class PacketUtilImpl implements PacketListener {
        ...
        public PacketUtilImpl(JavaPlugin plugin) {
            allPackets = ListeningWhitelist.newBuilder().normal().gamePhase(GamePhase.BOTH).types(handlers.keySet())
                    .build();
            this.parent = plugin;
        }
        ...
        public void onPacketSending(PacketEvent e) {
            if (handlers.containsKey(e.getPacketType()))
        ...
        @Override
        public ListeningWhitelist getSendingWhitelist() { return allPackets; }

        @Override
        public ListeningWhitelist getReceivingWhitelist() { return allPackets; }

        @Override
        public org.bukkit.plugin.Plugin getPlugin() { return parent; }
    }
