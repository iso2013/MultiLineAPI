...
public class PacketListener implements Listener {
    ...
    public PacketListener(MultiLineAPIPlugin parent, Map<Integer, TagImpl> entityTags, VisibilityStates visibilityState,
    ...
    public void onEvent(EntityPacketEvent e) {
        ...
        TagImpl tag = entityTags.get(identifier.getEntityID());
        if (tag == null) return;
    ...
    private void checkDataInvisible(EntityDataPacket dataPacket, TagImpl tag, Player target) {
        TagRenderer renderer = tag.getRenderer();
        ModifiableEntity modifiable = packet.wrap(dataPacket.getMetadata());
    ...
    private void checkMount(EntityMountPacket mountPacket, EntityIdentifier identifier, TagImpl tag, Player target) {
    ...
    private void manageDestroyPacket(EntityDestroyPacket destroyPacket, Player player) {
        Set<TagImpl> possibleDeletions = new HashSet<>();
        ...
        Bukkit.getScheduler().runTaskLater(parent, () -> {
            for (TagImpl tag : possibleDeletions) {
